...
ax.set_yscale("log")
ax.set_xlabel(f"IsoRank alignment score (raw, not log-transformed; bucket width {BIN_WIDTH:g})")
ax.set_ylabel("Count of aligned pairs (log scale)")
ax.grid(True, axis="y", color="#e6e6e3", linewidth=0.6)
ax.set_axisbelow(True)
for spine in ("top", "right"):
    ax.spines[spine].set_visible(False)
ax.set_title(f"IsoRank alignment scores, {a} <-> {b} — raw score, not log-transformed\n"
...
out = {
    "species_a": a, "species_b": b, "n_pairs": n, "bin_width": BIN_WIDTH,
