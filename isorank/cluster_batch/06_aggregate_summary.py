#!/usr/bin/env python3
"""
Collect every <a>_lcc_<b>_lcc_alignment_scores.json produced by
...
Expects species.tsv and ../<a>_lcc_<b>_lcc_alignment_scores.json for each pair.
...
rows = []
for path in sorted(glob.glob(os.path.join(ISORANK_DIR, "*_lcc_*_alignment_scores.json"))):
    with open(path) as f:
        d = json.load(f)
    rows.append(d)
...
found = {tuple(sorted((r["species_a"].removesuffix("_lcc"), r["species_b"].removesuffix("_lcc"))))
         for r in rows}
...
with open(out_tsv, "w", newline="") as f:
    ...
    for r in rows:
        w.writerow([r["species_a"].removesuffix("_lcc"), r["species_b"].removesuffix("_lcc"),
