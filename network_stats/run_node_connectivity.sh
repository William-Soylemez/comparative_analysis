#!/bin/bash
#SBATCH --job-name=cluster_node_conn
#SBATCH --output=cluster_node_conn_%j.out
#SBATCH --error=cluster_node_conn_%j.err
...
# Compute cluster_graph.avg_node_connectivity_largest_cc for every species
# under BASE_DIR that doesn't already have it (currently null for 52 of 79
# species -- left that way by an old run's serial single-core 30-min budget
# check; ~673 CPU-hours of work remaining, ~4.7h wall-clock ideal on 144
# cores). --skip avg_edge_connectivity keeps this job from also computing
# the edge-connectivity average -- that's run_edge_connectivity.sh's job, so
# the two can be scheduled/rerun completely independently of each other.
#
...
# uneven (minutes to 90+ hours), so fanning across species would leave most
# cores idle while the single biggest species finishes alone on one core.
# Instead this gives EVERY core to ONE species at a time (--procs = the
# whole node, via parallel_average_connectivity's fork pool -- see
# compute_cluster_graph_stats.py), processing the most expensive species
# first.
#
# Safe to run independently of run_species_batch.sh / run_edge_connectivity.sh,
...
#
# Usage:
#   sbatch run_node_connectivity.sh <species_base_dir> <out_dir> [accessions_file]
#
...
BASE_DIR="${1:?Usage: sbatch run_node_connectivity.sh <species_base_dir> <out_dir> [accessions_file]}"
OUT_DIR="${2:?Usage: sbatch run_node_connectivity.sh <species_base_dir> <out_dir> [accessions_file]}"
ACCESSIONS_FILE="${3:-}"
...
if [[ -n "$ACCESSIONS_FILE" ]]; then
    : > "$OUT_DIR/node_conn_species_list.txt"
    while read -r acc; do
        ...
        for d in "$BASE_DIR/${acc}_results" "$BASE_DIR/${acc}"; do
            if compgen -G "$d/*_network.positive.tsv" > /dev/null; then
                echo "$d" >> "$OUT_DIR/node_conn_species_list.txt"
                found=1
                break
            fi
        done
...
# meta, present even for species that still have a null connectivity value,
# since the size estimate is made before any skip/budget decision) so the
# most expensive species run first -- if the job times out, only cheap
...
python3 - "$OUT_DIR" < "$OUT_DIR/node_conn_species_list.txt" > "$OUT_DIR/node_conn_species_list.sorted.txt" <<'PY'
import json, os, sys
...
for line in sys.stdin:
    ...
    try:
        ...
        meta = cg.get("avg_node_connectivity_meta") or cg.get("avg_edge_connectivity_meta") or {}
...
for _, d in rows:
    print(d)
PY
mv "$OUT_DIR/node_conn_species_list.sorted.txt" "$OUT_DIR/node_conn_species_list.txt"

N_SPECIES=$(wc -l < "$OUT_DIR/node_conn_species_list.txt")
...
while read -r dir; do
    ...
    if python3 "$SCRIPT_DIR/compute_network_stats.py" "$dir" --quiet \
            --skip avg_edge_connectivity \
            --out "$out" --procs "$JOBS" \
            > "$OUT_DIR/logs/${acc}_node_conn.log" 2>&1; then
        echo "  done:  $acc"
    else
        echo "  FAILED: $acc (see $OUT_DIR/logs/${acc}_node_conn.log)"
    fi
    echo "  elapsed so far: $(( $(date +%s) - t_start ))s"
done < "$OUT_DIR/node_conn_species_list.txt"
